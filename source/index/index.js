...
class Index extends React.Component {
    ...
    procRender() {
        ...
        for (let key in list) {
            list[key].proc.map((item)=> {
                procList.push(
                    <tr key={item.pid}>
                        <td>{item.name}</td>
                        <td>{item.pm_id}</td>
                        <td>{item.pm2_env.exec_mode}</td>
                        <td>{item.pid}</td>
            ...
            series: [
                {
                    ...
                    data: [
                        {value: ((this.state.sysInfo.totalmem - this.state.sysInfo.freemem)/(1024*1024)).toFixed(2), name: '已使用'},
                ...
                <div className="earth-container col-xs-12">
                    <div className="col-xs-8 text-center">
                        <div className=" col-xs-12">
                            <i className={heartbeat[key] === 1 ?'earth health-earth':'earth dangerous-earth'}>&#xe600;</i>
                        </div>
                    </div>
                    <div className="col-xs-4 text-center">
                        <span className={heartbeat[key]===1?'font26 inline mgt30':'font26 dangerous-earth'}>{key}</span>
                        {heartbeat[key] === 1 ?
    ...
    render() {
        let {sysInfo, list}=this.state, state = this.state,
            {totalmem, freemem,loadavg}=sysInfo, appNum = 0, appNames = '';
        for (let key in list) {
            appNum++;
            appNames += ' '+key;
        }
                ...
                <div>
                    ...
                    <div className="col-xs-8">
                        ...
                        <div className="col-xs-12 mgt10">
                            <div className="col-xs-8 process-body">
